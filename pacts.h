...
PTH_DEF void pth_str_chop_last(char *str, int n) {
...
PTH_DEF void pth_checked_vsnprintf(char *str, const char *restrict format, ...) {
    ...
    va_end(args);
    assert((0 <= r && r+1 < PTH_PATH_MAX_SIZE) && "vsnprintf failed");
}
...
PTH_DEF void pth_join(Path *p, char *str) {
    pth_checked_vsnprintf(p->str+strlen(p->str), "%s%s", PTH_SEP, str);
}
